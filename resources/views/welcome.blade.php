...
:root{
    ...
    *{box-sizing:border-box}
    ...
    body{font-family:Inter,system-ui; color:#0b1220; background:
    ...
    header{position:sticky;top:0;z-index:50;background:rgba(255,255,255,.85);backdrop-filter:saturate(150%) blur(8px);border-bottom:1px solid #e5e7eb}
    ...
    .btn-ghost{border-color:#e5e7eb;color:#334155;background:white}
    ...
    h2{font-size:clamp(24px,3.2vw,34px);margin:0 0 10px;color:var(--ink)}
    p{line-height:1.6}
    ...
    .icon{width:36px;height:36px;border-radius:10px;background:#fff0f0;color:var(--brand);display:grid;place-items:center;margin-bottom:10px}
    .pill{display:inline-block;padding:6px 10px;border-radius:999px;background:#eaf7f0;color:#0f5132;font-weight:700;font-size:12px}
    .steps{display:grid;grid-template-columns:repeat(4,1fr);gap:16px}
    .step{background:linear-gradient(180deg,#fff, #fff5f5);border:1px solid #ffd6d6;border-radius:14px;padding:18px}
    ...
    details{border:1px solid #e5e7eb;border-radius:12px;padding:14px 16px;background:#fff}
    details+details{margin-top:10px}
    summary{font-weight:700;cursor:pointer}
    .contact-wrap{display:grid;grid-template-columns:1.1fr .9fr;gap:24px}
    textarea{min-height:110px;resize:vertical}
    footer{padding:30px 0;border-top:1px solid #e5e7eb;color:var(--muted)}
    /* Modal auth */
    .modal-backdrop{position:fixed;inset:0;background:rgba(2,8,23,.5);display:none;align-items:center;justify-content:center;padding:20px;z-index:60}
    .modal{width:min(560px,100%);background:white;border:1px solid #e5e7eb;border-radius:16px;box-shadow:0 20px 50px rgba(2,8,23,.25);padding:18px}
    .tabs{display:flex;gap:8px;margin-bottom:12px}
    .tabs button{flex:1} .hidden{display:none !important}
  ...
  <div id="authModal" class="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="authTitle">
    <div class="modal">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
        <h3 id="authTitle" style="margin:0;color:var(--ink)">Compte candidat</h3>
        <button id="authClose" class="btn btn-ghost" aria-label="Fermer">✕</button>
      </div>
      <div class="tabs">
        <button id="tabSignup" class="btn btn-outline" type="button">Créer un compte</button>
        <button id="tabLogin" class="btn btn-ghost" type="button">Se connecter</button>
      </div>

      <!-- Signup -->
      <form id="signupForm">
        <div class="auth-row">
          <div class="field"><label for="firstName">Prénom</label><input id="firstName" class="input" required></div>
          <div class="field"><label for="lastName">Nom</label><input id="lastName" class="input" required></div>
        </div>
        <div class="field"><label for="signupEmail">Email</label><input id="signupEmail" class="input" type="email" required></div>
        <div class="field"><label for="signupPwd">Mot de passe (≥ 6 caractères)</label><input id="signupPwd" class="input" type="password" minlength="6" required></div>
        <button class="btn btn-primary" type="submit" style="width:100%">Créer mon compte</button>
        <p id="signupMsg" class="muted" style="margin-top:8px"></p>
      </form>

      <!-- Login -->
      <form id="loginForm" class="hidden">
        <div class="field"><label for="loginEmail">Email</label><input id="loginEmail" class="input" type="email" required></div>
        <div class="field"><label for="loginPwd">Mot de passe</label><input id="loginPwd" class="input" type="password" minlength="6" required></div>
